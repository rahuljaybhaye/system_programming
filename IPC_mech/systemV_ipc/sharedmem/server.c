       ...
       int shmget(key_t key, size_t size, int shmflg);
			...
			3rd arg:  shared mem. flags
				...
				IPC_CREAT :-   Create a new segment.
							   If this flag is not used, then shmget() will find  the  segment associated
...
int main()
{
	...
	// Create shared mem. segment(Physical frame) in RAM
	shmid = shmget(KEY,20,IPC_CREAT|0640);

	printf("shmget return value id : %d\n",shmid);

	//Attach created physical frame by shmget() to page table
	shmaddr = shmat(shmid,NULL,0);

	printf("shmat return, Shared mem addr %x\n",shmaddr);

	// Data writing to half part of physical frame
	strcpy(shmaddr,"SSGM Electronics Pvt. ltd.");
	printf("data write to mem\n");
}
